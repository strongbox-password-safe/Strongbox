...
{
    return [_storageProvider isCloudBased];
}
    ...
    if(!_isUsingOfflineCache)
    {
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void)
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^(void){
                ...
                {
                    handler([Utils createNSError:@"Could not get safe as data." errorCode:-1]); // TODO: Improve info here?
                }
                
                [_storageProvider update:self.metadata data:updatedSafeData viewController:viewController completionHandler:^(NSError* error){
...
-(void) saveOfflineCacheFile:(NSData*)data safe:(SafeMetaData*)safe localProvider:(LocalDeviceStorageProvider*)localProvider
{
    ...
    {
        [localProvider updateOfflineCachedSafe:safe data:data viewController:nil completionHandler:^(NSError *error) {
    ...
    else
    {
        ...
        [localProvider create:safe.offlineCacheFileIdentifier data:data parentReference:nil viewController:nil completionHandler:^(NSString *fileName, NSString *fileIdentifier, NSError *error) {
            [self onStoredOfflineCacheFile:safe error:error];
        }];
    }
}
...
{
    [_local deleteOfflineCachedSafe:_metadata completionHandler:^(NSError *error) {
