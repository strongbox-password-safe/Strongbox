...
- (instancetype)initNewWithPassword:(NSString *)password {
    return [self initWithSafeDatabase:[[SafeDatabase alloc] initNewWithPassword:password]];
}

- (instancetype)initExistingWithDataAndPassword:(NSData *)data password:(NSString *)password error:(NSError **)ppError {
    return [self initWithSafeDatabase:[[SafeDatabase alloc] initExistingWithData:password data:data error:ppError]];
}
