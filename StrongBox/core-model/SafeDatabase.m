...
-(SafeDatabase*)initExistingWithData:(NSString*)password
                       ...
                       error:(NSError**)ppError
{
    ...
    //for(Field *hdr in headerFields){
    //    NSLog(@"OPEN HDR: %@ -> %@", hdr.prettyTypeString, hdr.prettyDataString);
    //}
    ...
    {
        NSLog(@"HMAC is no good! Corrupted Safe!!1");
...
-(NSData*) getAsData
{
    ...
    NSData *K, *L;
    PasswordSafe3Header hdr = [SafeTools generateNewHeader: _masterPassword K:&K L:&L];
    ...
    Field *hdrEnd = [[Field alloc] initNewDbHeaderField:HDR_END];
    ...
    for(Record *record in _records)
    {
        ...
        Field *end = [[Field alloc] initNew:FIELD_TYPE_END];
...
- (void)setLastUpdateUser
{
    ...
    // TODO: Refactor so this is not done here... should be passed in
    
#if TARGET_OS_IPHONE
    // Update User - Hard to get actual user name so use Device name instead, it usually be something like John's iPhone
    NSString* deviceName = [[UIDevice currentDevice] name];
#else
    NSString* deviceName = NSUserName();
#endif
    
    if(appField)
    {
        [appField setDataWithString:deviceName];
    }
    else
    {
        Field *lastUpdateUser =[[Field alloc] initNewDbHeaderField:HDR_LASTUPDATEUSER withString:deviceName];
