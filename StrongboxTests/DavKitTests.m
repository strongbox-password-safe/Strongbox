...
- (void)testUpdate {
    ...
    [provider create:@"Hello"
      ...
      viewController:nil
          completion:^(SafeMetaData *metadata, NSError *error) {
        [provider pushDatabase:metadata interactiveVC:nil data:[@"Another test...." dataUsingEncoding:NSUTF8StringEncoding] completion:^(StorageProviderUpdateResult result, NSDate * _Nullable newRemoteModDate, const NSError * _Nullable error) {
            NSLog(@"Update: %@", error);
            self.done = YES;
        }];
    }];
