...
class CompareDatabasesViewController: NSViewController {
    ...
    @objc func onTableViewClicked() {
        guard tableView.clickedRow != -1,
              ...
              let view = tableView.view(atColumn: 0, row: tableView.clickedRow, makeIfNecessary: false) else
        {
            return
        }
    ...
    func loadRows() {
        guard let diffSummary = diffSummary else {
            return
        }
    ...
    func getIconForNode(_ node: Node) -> IMAGE_TYPE_PTR {
        return NodeIconHelper.getIconFor(node, predefinedIconSet: firstModel.metadata.iconSet, format: firstModel.originalFormat, large: false)
    }
    ...
    @IBAction func onMergeSynchronize(_ sender: Any) {
...
extension CompareDatabasesViewController: NSTableViewDataSource, NSTableViewDelegate {
    ...
    func dereference(text: String, node: Node) -> String {
        return firstModel.dereference(text, node: node)
    }
}
